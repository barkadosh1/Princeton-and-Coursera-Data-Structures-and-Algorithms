...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 ...
	 // getDistance2 is my implementation 
	 public List<Integer> getDistance2(int v) {
		
		 List<Integer> onehop = new ArrayList<Integer>();
		 List<Integer> twohop = new ArrayList<Integer>();	
		 List<Integer> twohopfull = new ArrayList<Integer>();
		 
		 onehop = getNeighbors(v);
		 for (int i = 0; i < onehop.size(); i++) {
			 twohop = getNeighbors(onehop.get(i));
			 for (int j = 0; j < twohop.size(); j++) {
				 twohopfull.add(twohop.get(j));
			 }
			 }
